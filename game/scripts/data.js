export default async function loadResources(tables, urls, onLoadCallBack) {
    try {
        await loadJSONS(tables, urls);
        // TODO: Preload images
        onLoadCallBack();
    } catch (error) {
...
// Designed to extract data from a json representing an array of objects.
async function loadJSONS(tables, urls) {
    for (let i = 0; i < tables.length; i++) {
        const jsonData = await fetch(urls[i]);

        if (jsonData.status !== 200 && !jsonData.ok) {
            throw new Error(`${jsonData.status}: Unable to fetch ${urls[i]}`)
        }

        const dataArray = await jsonData.json()

        dataArray.forEach(element => {
            tables[i].push(element);
        });

        console.log(`Table ${i} LOADED.`, tables[i])
    }
    ...
    // tables["intelligentRacesTable"] = await responseIntelligentRaces.json()
    ...
    // tables.strongTraitsTable = await responseStrongTraits.json()
    // console.log("Strong Traits Table LOADED");
    // console.log(tables.strongTraitsTable);
    ...
    // tables.monstersTable = await responseMonsters.json()
    ...
    // tables.scopaDeck = await responseScopaDeck.json()
    // shuffle(tables.scopaDeck)
    ...
    // tables.coinsItemsTable = await responseCoinsItems.json()
    ...
    // tables.swordsItemsTable = await responseSwordsItems.json()
    ...
    // tables.clubsItemsTable = await responseClubsItems.json()
    ...
    // tables.cupsItemsTable = await responseCupsItems.json()
    ...
    // tables.weakTraitsTable = await responseWeakTraits.json()
    ...
    // tables.encountersTable = await responseEncounters.json()
    ...
    // tables.environmentsTable = await responseEnvironments.json()
    ...
    // tables.strongissimeTraitsTable = await responseStrongissimeTraits.json()
    ...
    // tables.bossesTable = await responseBosses.json()
